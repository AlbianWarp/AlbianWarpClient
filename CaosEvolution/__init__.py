...
if (platform.system() == "Linux"):
    ...
    def tryPort(port):
        try:
            ...
            r = i.ExecuteCaos("setv va00 9  mulv va00 va00  outv va00")    # Square of 9 == 81  :>
    ...
    if (os.path.isfile(f)):
        ...
        try:
            ...
            print("Found standard port file at "+f+" but couldn't parse it as an integer!  It contained: "+repr(p))
    ...
    tlc2esWithPorts = filter(lambda t: t.get("e6b02a88-7311-4a27-bbb7-d8f3a2d4e353") != None and t["e6b02a88-7311-4a27-bbb7-d8f3a2d4e353"].get("caosInjectionType") == "tcp" and t["e6b02a88-7311-4a27-bbb7-d8f3a2d4e353"].get("caosInjectionPort") != None, tlc2es)
    
    if (not showHiddens):
        tlc2esWithPorts = filter(lambda t: not (t.get("e6b02a88-7311-4a27-bbb7-d8f3a2d4e353") != None and t["e6b02a88-7311-4a27-bbb7-d8f3a2d4e353"].get("hidden") == True), tlc2esWithPorts)
    ...
    menu.extend(sorted(map(lambda t: (t["e6b02a88-7311-4a27-bbb7-d8f3a2d4e353"]["caosInjectionPort"], formatTLC2EInstance(t)), tlc2esWithPorts), key=lambda a: a[1]))
    ...
    if (len(menu) == 0):
        if (standardPort != None):
            if (standardPort == ParseError):
                print()
                print("Uh oh, we need for this file to contain the TCP port number of the engine to inject CAOS through (an integer, not "+repr(p)+"): "+f)
                print("We also tried TLC2E and the default port "+str(DefaultPort)+", but couldn't find an engine on either!")
                print("So I have no idea which engine to connect to if one is even running :[")
                sys.exit(33)
            else:
                print()
                print("We tried TLC2E, the port "+str(standardPort)+" explicitly given by "+f+" and the default port "+str(DefaultPort)+", but couldn't find an engine on any of them!")
                print("So it looks like an engine isn't running, and if it is, then I don't know which one to connect to! :P")
                sys.exit(34)
        else:
            print()
            print("Uh oh, we need either TLC2E or this file to find the TCP loopback port to inject CAOS into the engine through: "+f)
            print("We also tried the default port "+str(DefaultPort)+", but couldn't find an engine on it!")
            print("So I have no idea which engine to connect to if one is even running :[")
            sys.exit(32)
        ...
        while True:
            ...
            if (ai == None):
                print("  I'm sorry I didn't understand "+repr(a)+".  Please enter an integer ^^'")
            else:
                if (ai < 0 or ai >= len(menu)):
                    print("  I'm sorry, please enter an integer on the menu ^^'")
                else:
                    port = menu[ai][0]
                    print("Excellent!  Using engine at port "+str(port)+": "+menu[ai][1]+"  :D")
                    break
    #
...
# The windows one!
    ...
    #### Bring in the native code (the class "CaosInjector") ####
    sys.path.append('.')
    import clr
